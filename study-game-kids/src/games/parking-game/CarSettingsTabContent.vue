...
const currentCar = computed(() => {
    ...
    if (!p) return { body: 0x0164e8, tires: 0x333333 };
    return props.team === 'P1' ? p.p1Car : p.p2Car;
});
...
const activeColor = computed({
    ...
    set: (val) => {
        const p = { ...settingsStore.gameSpecificSettings['parking-game'] };
        if (props.team === 'P1') {
            p.p1Car = { ...p.p1Car, [subTab.value]: val };
        } else {
            p.p2Car = { ...p.p2Car, [subTab.value]: val };
        }
        settingsStore.updateGameSpecificSettings('parking-game', p);
    }
});
